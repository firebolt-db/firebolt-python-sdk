...
def test_connection_token_caching(
    ...
    with Patcher():
        with connect(
            ...
            assert connection.cursor().execute("select*") == len(python_query_data)
        # Verify token was cached using the new cache system (second test)
        cached_token = get_cached_token(client_id, client_secret, account_name)
        assert cached_token == access_token, "Invalid token value cached"
...
def test_connection_commit(
    connection: Connection,
    httpx_mock: HTTPXMock,
    simple_commit_callback: Callable,
):
...
def test_connection_autocommit_property(connection: Connection):
...
def test_in_transaction_property_reflects_transaction_state(connection: Connection):
...
def test_transaction_id_parsed_from_server_response_headers(
    httpx_mock: HTTPXMock,
...
def test_transaction_sequence_id_parsed_from_server_response_headers(
    httpx_mock: HTTPXMock,
    connection_autocommit_off: Connection,
    begin_transaction_callback: Callable,
...
def test_transaction_params_included_in_subsequent_requests(
    httpx_mock: HTTPXMock,
    connection_autocommit_off: Connection,
    begin_transaction_callback: Callable,
    transaction_query_callback: Callable,
    ...
    cursor.execute("SELECT 1")  # This will fail if transaction params aren't passed


def test_reset_session_header_clears_transaction_state(
    httpx_mock: HTTPXMock,
    connection_autocommit_off: Connection,
    begin_transaction_callback: Callable,
    transaction_query_callback: Callable,
...
def test_remove_parameters_header_clears_transaction_state(
    httpx_mock: HTTPXMock,
    connection_autocommit_off: Connection,
...
def test_connection_context_manager_handles_transaction_cleanup(
    httpx_mock: HTTPXMock,
    auth: Auth,
    account_name: str,
    api_endpoint: str,
