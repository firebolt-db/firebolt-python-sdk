...
class TestStreamingAsyncRowSet:
    """Tests for StreamingAsyncRowSet functionality."""

    @pytest.fixture
    def streaming_rowset(self):
        """Create a fresh StreamingAsyncRowSet instance."""
        return StreamingAsyncRowSet()

    @pytest.fixture
    def mock_response(self):
        """Create a mock Response with valid JSON lines data."""
        mock = MagicMock(spec=Response)
        mock.aiter_lines.return_value = self._async_iter(
            [
                json.dumps(
                    {
                        "message_type": "START",
    ...
    # Helper method to create async iterators for testing
    async def _async_iter(self, items):
        for item in items:
            yield item

    async def test_init(self, streaming_rowset):
    ...
    async def test_append_empty_response(self, streaming_rowset):
    ...
    @patch(
        "firebolt.common.row_set.asynchronous.streaming.StreamingAsyncRowSet._fetch_columns"
    )
    async def test_append_response(
        self, mock_fetch_columns, streaming_rowset, mock_response
    ):
        ...
        await streaming_rowset.append_response(mock_response)
    ...
    @patch(
        "firebolt.common.row_set.asynchronous.streaming.StreamingAsyncRowSet._next_json_lines_record"
    )
    @patch(
    ...
    async def test_fetch_columns(
        ...
        columns = await streaming_rowset._fetch_columns()
    ...
    async def test_fetch_columns_empty_response(self, streaming_rowset):
    ...
    async def test_fetch_columns_unexpected_end(
        ...
        with pytest.raises(OperationalError) as err:
            await streaming_rowset._fetch_columns()
    ...
    async def test_nextset_no_more_sets(
        self,
        mock_fetch_columns_from_record,
        mock_next_json_lines_record,
        streaming_rowset,
        ...
        assert await streaming_rowset.nextset() is False

    @patch(
        "firebolt.common.row_set.asynchronous.streaming.StreamingAsyncRowSet._fetch_columns"
    )
    async def test_nextset_with_more_sets(self, mock_fetch_columns, streaming_rowset):
        ...
        assert await streaming_rowset.nextset() is True
    ...
    async def test_iteration(self, streaming_rowset):
        ...
        with patch.object(
            ...
            mock_get_next_row.side_effect = expected_rows + [StopAsyncIteration()]
            ...
            mock_pop_data_record.return_value = data_record
            ...
            rows.append(await streaming_rowset.__anext__())
            rows.append(await streaming_rowset.__anext__())

            # Verify the StopIteration is raised after all rows are consumed
            with pytest.raises(StopAsyncIteration):
                await streaming_rowset.__anext__()
    ...
    async def test_iteration_empty_response(self, streaming_rowset):
    ...
    async def test_error_response(self, streaming_rowset, error_record):
    ...
    async def test_aclose(self, streaming_rowset, mock_response):
        """Test aclose method."""
        response1 = MagicMock(spec=Response)
        ...
        # Close the row set
        await streaming_rowset.aclose()

        # Verify all responses are closed
        response1.aclose.assert_called_once()
        response2.aclose.assert_called_once()

        # Verify internal state is reset
        assert streaming_rowset._responses == []

    async def test_aclose_with_error(self, streaming_rowset):
        """Test aclose method when response closing raises an error."""
        response = MagicMock(spec=Response)
        response.is_closed = False
        response.aclose.side_effect = HTTPError("Test error")
    ...
    async def test_close_on_error_context_manager(self, streaming_rowset):
        """Test _close_on_op_error context manager."""
        # Create an awaitable mock for aclose method
        async def mock_aclose():
            pass

        streaming_rowset.aclose = MagicMock()
        streaming_rowset.aclose.side_effect = mock_aclose

        # When no error occurs, close should not be called
        async with streaming_rowset._close_on_op_error():
            pass
        streaming_rowset.aclose.assert_not_called()

        # When OperationalError occurs, close should be called
        with pytest.raises(OperationalError):
            async with streaming_rowset._close_on_op_error():
                raise OperationalError("Test error")
        streaming_rowset.aclose.assert_called_once()

    async def test_next_json_lines_record_none_response(self, streaming_rowset):
        """Test _next_json_lines_record with None response."""
        streaming_rowset.append_empty_response()

        assert await streaming_rowset._next_json_lines_record() is None

    @patch(
        "firebolt.common.row_set.asynchronous.streaming.StreamingAsyncRowSet._fetch_columns"
    )
    async def test_next_json_lines_record_http_error(
        self, mock_fetch_columns, streaming_rowset
    ):
        """Test _next_json_lines_record when aiter_lines raises HTTPError."""
        mock_fetch_columns.return_value = []

        response = MagicMock(spec=Response)
        response.aiter_lines.side_effect = HTTPError("Test error")

        streaming_rowset._responses = [response]

        with pytest.raises(OperationalError) as err:
            await streaming_rowset._next_json_lines_record()
    ...
    async def test_next_data_record_navigation(self, mock_get_next, streaming_rowset):
        """Test __anext__ record navigation logic."""
        ...
        # Create an awaitable version of pop_data_record for the second part of the test
        async def mock_pop_data_record():
            return new_record

        # Call __anext__
        result = await streaming_rowset.__anext__()
        ...
        streaming_rowset._pop_data_record = MagicMock()
        streaming_rowset._pop_data_record.side_effect = mock_pop_data_record

        # Call __anext__ again
        await streaming_rowset.__anext__()
    ...
    async def test_iteration_stops_after_response_consumed(self, streaming_rowset):
    ...
    async def test_pop_data_record_from_record_unexpected_end(self):
    ...
    async def test_corrupted_json_line(self, streaming_rowset):
        ...
        with patch(
            ...
            mock_response = MagicMock(spec=Response)
            mock_response.aiter_lines.return_value = self._async_iter(
                [
                    json.dumps(
                        {
                            "message_type": "START",
    ...
    async def test_malformed_record_format(self, streaming_rowset):
    ...
    async def test_recovery_after_error(self, streaming_rowset):
        """Test recovery from errors when multiple responses are available."""
        with patch(
            "firebolt.common.row_set.streaming_common.parse_json_lines_record"
        ) as mock_parse, patch.object(streaming_rowset, "aclose") as mock_close:
            ...
            mock_response1.aiter_lines.return_value = self._async_iter(
                [
            ...
            mock_response2.aiter_lines.return_value = self._async_iter(
                [
            ...
            with pytest.raises(OperationalError):
                ...
                await streaming_rowset._next_json_lines_record()  # This will raise

            # aclose() should be called by _close_on_op_error
            ...
            with patch.object(
                ...
                row = await streaming_rowset.__anext__()
                assert row == ["success"]
    ...
    async def test_unexpected_message_type(self, streaming_rowset):
    ...
    async def test_rows_returned_tracking(self, streaming_rowset):
        ...
        with patch(
            ...
            with patch.object(
                ...
                # Since we're manually calling __anext__() 5 times, we should get multiple calls to _pop_data_record
    ...
    async def test_multiple_response_error_cleanup(self, streaming_rowset):
        ...
        response1 = MagicMock(spec=Response)
        response1.is_closed = False
        response1.aclose.side_effect = HTTPError("Error 1")

        response2 = MagicMock(spec=Response)
        response2.is_closed = False
        response2.aclose.side_effect = HTTPError("Error 2")

        response3 = MagicMock(spec=Response)
        response3.is_closed = False
        response3.aclose.side_effect = HTTPError("Error 3")
        ...
        with patch.object(streaming_rowset, "_reset", side_effect=patched_reset):
            ...
            response1.aclose.assert_called_once()
            response2.aclose.assert_called_once()
            response3.aclose.assert_called_once()
